...
def read_users(db: Session = Depends(deps.get_db), skip: int = 0, limit: int = 100,
    ...
    Retrieve users.
    """
    users = crud.user_orm.get_multi(db, skip=skip, limit=limit)
    return users
...
def update_user_me(*, db: Session = Depends(deps.get_db), password: str = Body(None), full_name: str = Body(None),
    ...
    user = crud.user_orm.update(db, db_obj=current_user, obj_in=user_in)
    return user


@router.get("/me", response_model=schemas.User)
def read_user_me(db: Session = Depends(deps.get_db),
                 current_user: models.User = Depends(deps.get_current_active_user)) -> Any:
    """
    Get current user.
    """
...
def create_user_open(*, db: Session = Depends(deps.get_db), password: str = Body(...), email: EmailStr = Body(...),
    ...
    user = crud.user_orm.get_by_email(db, email=email)
    if user:
        raise HTTPException(status_code=400, detail="The user with this username already exists in the system")
...
def read_user_by_id(user_id: int, current_user: models.User = Depends(deps.get_current_active_user),
    ...
    """
    user = crud.user_orm.get(db, id=user_id)
    if user == current_user:
        return user

    if not crud.user_orm.is_superuser(current_user):
...
def update_user(*, db: Session = Depends(deps.get_db), user_id: int, user_in: schemas.UserUpdate,
    ...
    user = crud.user_orm.update(db, db_obj=user, obj_in=user_in)
    return user
